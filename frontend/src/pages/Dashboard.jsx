...
const Dashboard = () => {
  ...
  useEffect(() => {
    if (!user) return;
    const myQuery = query(collection(db, "notebooks"), where("ownerId", "==", user.uid));
    const unsubscribeMy = onSnapshot(myQuery, (snapshot) => {
      setMyNotebooks(snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() })));
    });
    const savedQuery = query(collection(db, "notebooks"), where("isSaved", "==", true));
    const unsubscribeSaved = onSnapshot(savedQuery, (snapshot) => {
      setOpenedNotebooks(snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() })));
    });
    return () => { unsubscribeMy(); unsubscribeSaved(); };
  }, [user]);
    ...
    <div className="min-h-screen bg-neutral-900 text-neutral-100 flex relative">
        ...
        <div className="flex flex-col items-center text-center mt-4">
          {user ? (
            ...
            <div className="animate-pulse flex flex-col items-center">
              ...
              <div className="h-4 w-24 bg-neutral-800 rounded mt-4" />
            </div>
          )}
        </div>

        <div className="flex flex-col gap-3 w-full mt-10">
          <button onClick={() => navigate("/profile")} className="w-full py-2 rounded-lg bg-neutral-800 hover:bg-neutral-700 transition">Profile</button>
          <button onClick={() => navigate("/notifications")} className="w-full py-2 rounded-lg bg-neutral-800 hover:bg-neutral-700 transition">Notifications</button>
          <button onClick={() => navigate("/explore")} className="w-full py-2 rounded-lg bg-neutral-800 hover:bg-neutral-700 transition">Explore</button>
          <button onClick={async () => { await auth.signOut(); navigate("/login"); }} className="w-full py-2 rounded-lg bg-neutral-800 hover:bg-neutral-700 transition">Logout</button>
        </div>
      </aside>
      ...
      <main className="flex-1 p-10 relative">
        ...
        <button
          ...
          className="fixed bottom-10 right-10 w-16 h-16 rounded-full bg-emerald-500 hover:bg-emerald-600 shadow-2xl flex items-center justify-center text-4xl font-bold text-white transition transform hover:scale-105 active:scale-95"
